...
ddi_dma_attr_t bnxt_dma_attr = {
	...
	.dma_attr_sgllen = BNXT_DMA_SGLLEN,
...
{
	if (bdb->ncookies > 0) {
		VERIFY3P(bdb->dma_handle, !=, NULL);
		(void) ddi_dma_unbind_handle(bdb->dma_handle);
		bdb->ncookies = 0;
		bzero(&bdb->cookie, sizeof (ddi_dma_cookie_t));
		bdb->len = 0;
	}

	if (bdb->acc_handle != NULL) {
		ddi_dma_mem_free(&bdb->acc_handle);
		bdb->acc_handle = NULL;
		bdb->va = NULL;
	}

	if (bdb->dma_handle != NULL) {
		ddi_dma_free_handle(&bdb->dma_handle);
		bdb->dma_handle = NULL;
	}

	ASSERT3P(bdb->va, ==, NULL);
	ASSERT3U(bdb->ncookies, ==, 0);
	ASSERT3U(bdb->len, ==, 0);
}
    ...
    size_t size)
{
	...
	memcb = DDI_DMA_SLEEP;
	ret = ddi_dma_alloc_handle(bp->dip, &bnxt_dma_attr, memcb, NULL,
	    &bdb->dma_handle);
	if (ret != 0) {
		bnxt_log(bp, "!failed to allocate DMA memory: %d", ret);
		bdb->dma_handle = NULL;
		return (1);
	}

	ret = ddi_dma_mem_alloc(bdb->dma_handle, size, accp, flags,
	    memcb, NULL, &bdb->va, &len, &bdb->acc_handle);
	if (ret != DDI_SUCCESS) {
		bnxt_log(bp, "!failed to allocate DMA memory: %d", ret);
		bdb->va = NULL;
		bdb->acc_handle = NULL;
	...
	ret = ddi_dma_addr_bind_handle(bdb->dma_handle, NULL,
	    bdb->va, len, DDI_DMA_RDWR | flags, memcb, NULL,
	    &bdb->cookie, &bdb->ncookies);
	if (ret != DDI_DMA_MAPPED) {
		bdb->ncookies = 0;
		bzero(&bdb->cookie, sizeof (ddi_dma_cookie_t));
		bnxt_log(bp, "!failed to bind DMA memory: %d", ret);
		bnxt_dma_free(bdb);
		return (1);
	}

	bdb->len = size;
	bdb->pa = bdb->cookie.dmac_laddress;
...
bnxt_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	for (int i = 0; i < bp->ntxq; i++) {
		...
		bnxt_log(bp, "!txr%d %p", txr->idx, txr);
	}

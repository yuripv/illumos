...
static ntb_drvr_ops_t ntb_svc_ops;
...
init_ntb_trans(ntb_trans_t *ntb)
{
	...
	if ((ntb->status_tq = ddi_taskq_create(node->dip, tq_name, 1,
		...
		cmn_err(CE_WARN, "!failed to create taskq %s", tq_name);
		goto failed;
	}
	...
	if (dquery.dq_num_channels > 0 && !disable_dcopy) {
		...
		if (dcopy_alloc(DCOPY_SLEEP, &ntb->b_dcopy_hdl) !=
		    DCOPY_SUCCESS) {
			/*
		...
		if (ntb->dcopy_ena && dcopy_alloc(DCOPY_SLEEP,
	...
	if (ntb->dcopy_ena) {
		cmn_err(CE_NOTE, "!using dcopy/IOAT for blocks >= %d bytes",
...
failed:
	if (ntb->status_tq != NULL)
		ddi_taskq_destroy(ntb->status_tq);
	if (ntb->b_dcopy_hdl != NULL)
		dcopy_free(&ntb->b_dcopy_hdl);
	if (ntb->k_dcopy_hdl != NULL)
...
ntb_trans_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if (ntb_node != NULL) {
		cmn_err(CE_WARN, "!only one device is permitted");
		return (DDI_FAILURE);
	}
	 ...
	 */
	if (ntb_register(dip, &ntb_svc_ops, ntb_node) != DDI_SUCCESS) {
...
static struct bus_ops ntb_bus_ops = {
	...
	.bus_fm_init = 0,
	.bus_fm_fini = 0,
	.bus_fm_access_enter = 0,
	.bus_fm_access_exit = 0,
	.bus_power = 0,
